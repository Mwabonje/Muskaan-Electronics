        ...
        <div className="relative w-full max-w-md bg-white rounded-xl shadow-2xl flex flex-col max-h-full overflow-hidden print:shadow-none print:w-full print:max-w-none print:rounded-none">
          ...
          <div className="flex-1 overflow-y-auto p-8 bg-white text-slate-800 print:overflow-visible print:p-4">
            <div className="text-center mb-6">
              <h1 className="text-2xl font-black text-slate-900 mb-1">MUSKAAN ELECTRONICS</h1>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <div className="relative w-full max-w-2xl bg-[#0B1120] rounded-xl shadow-2xl border border-slate-800 flex flex-col max-h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6 custom-scrollbar">
              ...
              {cartItems.map((item, index) => (
                <div key={item.id} className="p-4 bg-[#1e293b]/30 border border-slate-800 rounded-xl space-y-4">
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1 space-y-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Price (Ksh)</label>
                      <input 
                        type="number" 
                        value={item.price}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <span className="text-slate-500 text-sm">{discountType === 'percent' ? '%' : 'Ksh'}</span>
                </div>
                <input 
                  type="number" 
                  value={discountValue}
          ...
          <div className="space-y-3">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
              <CreditCard className="w-3 h-3" /> Payment Method <span className="text-rose-500">*</span>
            </label>
          ...
          <div className="bg-blue-900/10 border border-blue-900/30 rounded-lg p-4 flex gap-3 items-start">
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-bold text-blue-500 uppercase tracking-wider">Sale Summary</p>
              <p className="text-sm font-mono text-slate-300">
                Items: {itemsText} • Payment: {paymentMethod} • Total: Ksh {grandTotal.toLocaleString()}
              </p>
            </div>
          </div>
