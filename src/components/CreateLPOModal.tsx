...
export default function CreateLPOModal({
  isOpen,
  onClose,
  lpoToEdit,
}: CreateLPOModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (lpoToEdit) {
        setSupplierName(lpoToEdit.supplierName);
        setExpectedDeliveryDate(lpoToEdit.expectedDeliveryDate || "");
        setNotes(lpoToEdit.notes || "");
        setCartItems(
          lpoToEdit.items.map((item) => ({
  ...
  const handleProductAdded = async (productId: number) => {
    ...
    setCartItems(prev => {
      ...
      if (lastItem && lastItem.productId === "") {
        // Replace empty last item
        const newItems = [...prev];
        newItems[newItems.length - 1] = { ...lastItem, productId, price };
        return newItems;
      } else {
        // Add new item
        return [...prev, { id: crypto.randomUUID(), productId, price, quantity: 1 }];
      }
    });
  };
  ...
  const handleConfirmLPO = async () => {
    ...
    try {
      const lpoData = {
        ...
        supplierName,
        date: lpoToEdit ? lpoToEdit.date : new Date().toISOString(),
      ...
      let lpoId;
      if (lpoToEdit && lpoToEdit.id) {
        await db.purchaseOrders.update(lpoToEdit.id, lpoData);
        lpoId = lpoToEdit.id;
      } else {
        lpoId = await db.purchaseOrders.add(lpoData);
      }
      ...
      <div className="relative w-full max-w-2xl bg-[#0B1120] rounded-xl shadow-2xl border border-slate-800 flex flex-col max-h-full overflow-hidden">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-[#0f172a]">
          <h2 className="text-lg font-bold text-white">
            {lpoToEdit ? "Edit Purchase Order (LPO)" : "Create Purchase Order (LPO)"}
          </h2>
          <button
            onClick={onClose}
                      ...
                      >
                        ...
                        {filteredProducts.map((p) => (
                          <option key={p.id} value={p.id}>
                            {p.name} - Cost: Ksh {formatPrice(p.cost)}
                          </option>
                        ))}
                      </select>
                    </div>
          ...
          <div className="bg-purple-900/10 border border-purple-900/30 rounded-lg p-4 flex gap-3 items-start">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm font-mono text-slate-300">
                Supplier: {supplierName || "Not set"} • Items: {itemsText} •
