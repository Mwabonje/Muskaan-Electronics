      ...
      <div className="relative w-full max-w-3xl bg-[#0B1120] rounded-xl shadow-2xl border border-slate-800 flex flex-col max-h-full overflow-hidden print:shadow-none print:w-full print:max-w-none print:rounded-none print:border-none print:bg-white">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800 bg-[#0f172a] print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">Purchase Order #LPO-{lpo.id?.toString().padStart(4, '0')}</h2>
              <p className="text-xs text-slate-400">{new Date(lpo.date).toLocaleString()}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 sm:p-8 space-y-8 custom-scrollbar bg-white text-slate-900 print:overflow-visible print:p-4">
          ...
          <div className="grid grid-cols-2 gap-8">
            ...
            <div className="space-y-4 text-right">
              <div className="space-y-1">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Order Details</p>
                <div className="flex items-center justify-end gap-2 text-slate-800 font-medium">
                  <span className="text-slate-500">LPO #:</span> LPO-{lpo.id?.toString().padStart(4, '0')}
                </div>
                <div className="flex items-center justify-end gap-2 text-slate-800 font-medium">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b-2 border-slate-200">
                  ...
                  <th className="py-3 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Unit Cost</th>
                  <th className="py-3 px-4 text-xs font-bold text-slate-500 uppercase tracking-wider text-right">Total</th>
                </tr>
              </thead>
                ...
                {lpo.items.map((item, index) => {
                  ...
                  return (
                    <tr key={index} className="group hover:bg-slate-50 transition-colors">
                      ...
                      <td className="py-4 px-4 text-sm text-slate-600 text-right">{formatPrice(unitCost)}</td>
                      <td className="py-4 px-4 text-sm font-bold text-slate-800 text-right">{formatPrice(total)}</td>
                    </tr>
                  );
                })}
              </tbody>
          ...
          <div className="mt-16 pt-8 border-t border-slate-200 grid grid-cols-2 gap-8">
            ...
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-8">Supplier Acceptance (Signature)</p>
              <div className="border-b border-slate-300 w-full"></div>
            </div>
          </div>
